...
describe("CapabilityAssembler", () => {
  describe("resolveEffectiveTools", () => {
    ...
    it("resolves multiple tools (2 bound, 1 disabled skipped)", async () => {
      const db = mockDb([
        makeBinding({ id: "b1", tool_ref: "echo" }),
        makeBinding({ id: "b2", tool_ref: "web_search" }),
        // Disabled bindings are filtered at the DB query level (WHERE enabled=true),
        // so they won't appear in the mock results.
      ])
      const assembler = new CapabilityAssembler({ db })
...
describe("CapabilityGuard", () => {
  it("wraps tool and preserves name/description/schema", () => {
    ...
    const wrapped = CapabilityGuard.wrap(
      tool,
      { agentId: "a1", jobId: "j1", userId: "u1" },
      { db },
    )
